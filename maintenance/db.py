...
print(f"* [{(time.time() - TS_START):08.5f}] {cur.rowcount} deleted 'deleted' post(s). (14 day delay for delete propagation/cache eviction)")
...
cur.execute("""UPDATE channels SET totalmsgcount = a.cnt, totalmsgcountroot = a.cntr
            ...
            WHERE channels.id = a.id and (totalmsgcount IS DISTINCT FROM a.cnt OR totalmsgcountroot IS DISTINCT FROM a.cntr)
            RETURNING *""")
...
cur.execute("""UPDATE channels SET lastpostat = a.lpts, lastrootpostat = a.lptsr
            ...
            WHERE channels.id = a.id and (lastpostat > a.lpts OR lastrootpostat > a.lptsr)
            RETURNING *""")
